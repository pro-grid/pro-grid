...
var progridserver = function (grid) {
  ...
  ioServer.sockets.on('connection', function (socket) {
    ApiKeyHandler.newKey(socket, null, function (socket) { // DIRTY FIX ME
      socket.emit('server ready', { gridArray: grid.gridMatrix, userColor: refreshColor() });
      userJoin();
      ...
      socket.on('refresh color', function() {
        socket.emit('color changed', { userColor: refreshColor() });
      });
